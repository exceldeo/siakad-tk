...
import app.siakad.siakadtkadmin.presentation.screens.registration.detail.unverified.RegistrationUnverDetailActivity
...
class RegistrationActivity : AppCompatActivity(), RegistrationClickHelper {
    ...
    override fun navigateToRegistrationDetail(daful: DaftarUlang) {
        if (daful.daful.statusDaful) {
            ...
            intent.putExtra(RegistrationListFragment.VERIFIED_REGISTRATIOIN, daful)
            startActivity(intent)
        } else {
            val intent = Intent(this, RegistrationUnverDetailActivity::class.java)
            intent.putExtra(RegistrationListFragment.UNVERIFIED_REGISTRATIOIN, daful)
            startActivity(intent)
        }
    }
}