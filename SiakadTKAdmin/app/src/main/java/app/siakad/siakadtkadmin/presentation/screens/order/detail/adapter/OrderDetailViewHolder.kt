...
class OrderDetailViewHolder(private val v: View) : RecyclerView.ViewHolder(v) {
  fun insertOrderDetail(item: DetailKeranjangModel, index: Int, type: String) {
    with(itemView) {
      ...
      if (type == OrderListFragment.ORDER_PENDING) {
        ...
        cb_item_order_detail_acc.visibility = View.GONE
      }
    }
  }
}